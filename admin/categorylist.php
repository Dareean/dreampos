		...
		<div class="page-wrapper">
			<div class="content">
				<div class="page-header">
					...
					<div class="page-btn">
						<a href="addcategory.php?uid=<?php echo $NewId ?>" class="btn btn-added"><img src="../assets/img/icons/plus.svg" alt="img" class="me-2">Add Category</a>
					</div>
				</div>
				...
				<div class="card">
					<div class="card-body">
						...
						<!-- /Filter -->
						<form id="exportExcelForm" method="post" action="processor/exportcategory.php">
									...
									<tbody>
										...
										if ($result->num_rows > 0) {
											...
											while ($row = $result->fetch_assoc()) {
												...
												echo "<td>
												<a class='me-3' href='editcategory.php?id=" . $row['id'] . "&uid=" . $NewId . "'>
	...
	<script>
		$(document).ready(function() {
			$('#export-excel').click(function(e) {
				...
				$('.datanew tbody input[type="checkbox"]:checked').each(function() {
					var row = $(this).closest('tr');
					var id = row.data('id'); // Retrieve the data-id attribute
					selectedIds.push(id);
				});
				...
				var form = $('<form action="processor/exportcategory.php" method="post" style="display: none;">');
	...
	<script>
		$(function() {
			$('#pdf').click(function() {
				...
				var _el = $('<div>');
				var _head = $('head').clone();
				_head.find('title').text("Category List - PDF View");
		...
		$(".ajax-delete-btn").on("click", function() {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					$.ajax({
						url: "./processor/deletecategory.php", // Replace with the correct URL
						type: "POST",
						data: {
							id: id
						},
