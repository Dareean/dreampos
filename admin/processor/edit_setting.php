...
if (isset($_POST['submit'])) {
    ...
    if ($old_image_result && $old_image_result->num_rows > 0) {
        ...
        if (!empty($_FILES["user_pic"]["tmp_name"]) && file_exists($_FILES["user_pic"]["tmp_name"])) {
            ...
            if (getimagesize($_FILES["user_pic"]["tmp_name"]) !== false) {
                ...
                if (move_uploaded_file($_FILES["user_pic"]["tmp_name"], $target_file)) {
                    ...
                    // You might want to handle this error scenario appropriately.
                }
            } else {
        ...
        // Update the database
        $update_sql = "UPDATE tbl_users SET name='$name', username='$username', email='$email', phone='$phone', status='$status', img_url='$profile_image_path' WHERE id='$user_id'";

        if ($conn->query($update_sql) === TRUE) {
            // Successfully updated
            // echo '<script>alert("Berhasil mengedit User."); window.location.href="../userlist.php?uid='.$NewId.'";</script>';
            exit();
        } else {
