...
if (isset($_POST['export_excel']) && isset($_POST['selected_ids'])) {
    ...
    if (!empty($sanitizedIds)) {
        ...
        $headers = array("No", "Brand Name", "Brand Description", "Created By");
        $sheet->fromArray($headers, null, 'A1');

        // Fetch data based on sanitized selected IDs
        $sql = "SELECT * FROM tbl_brands WHERE id IN (" . implode(',', $sanitizedIds) . ")";
        ...
        if ($result && $result->num_rows > 0) {
            ...
            $sheet->setCellValue('C' . $ro, $row['description']);
            
            // Add logic for 'Created By' column
            if ($row['id_user'] == 1) {
                $sheet->setCellValue('D' . $ro, "Admin");
            } elseif ($row['id_user'] == 2) {
                $sheet->setCellValue('D' . $ro, "Seller");
            } else {
                $sheet->setCellValue('D' . $ro, "Unknown");
            }
        ...
        header("Content-Disposition: attachment; filename=Data_Brands_List.xls");
