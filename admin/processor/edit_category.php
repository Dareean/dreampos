...
if (!isset($_POST['submit'])) {
    ...
    $get_old_image_sql = "SELECT img_url FROM tbl_categories ";
    ...
    if ($old_image_result && $old_image_result->num_rows > 0) {
        ...
        if (!empty($_FILES["category_pic"]["tmp_name"]) && file_exists($_FILES["category_pic"]["tmp_name"])) {
...
?>
                                ...
                                <?php if ($successMessage) : ?>
                                    Swal.fire({
                                        ...
                                        if (result.isConfirmed) {
                                            window.location.href = '../editcategory.php?uid=<?php echo $UniqueId; ?>&id=<?php echo $category['id'] ?>';
                                        }
                                    });
                                <?php endif; ?>
                            });
                ...
                if (move_uploaded_file($_FILES["category_pic"]["tmp_name"], $target_file)) {
                    $profile_image_path = "../../ims/assets/img/product/" . $file_name2;
                } else {
        ...
        $update_sql = "UPDATE tbl_categories SET name='$name', code='$code', description='$description', img_url='$profile_image_path' WHERE id='$category_id'";

        $submit = $conn->query($update_sql);

        if ($submit) {
            $_SESSION['info'] = "Categories updated successfully.";
        ...
        <?php
            ...
            $_SESSION['info'] = "Failed to update categories.";
            $successMessage = $_SESSION['info'] ?? null;
            unset($_SESSION['info']);
        ?>
