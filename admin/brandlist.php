		...
		<div class="page-wrapper">
			<div class="content">
				<div class="page-header">
					...
					<div class="page-btn">
						<a href="addbrand.php?uid=<?php echo $NewId ?>" class="btn btn-added"><img src="../assets/img/icons/plus.svg" class="me-2" alt="img">Add Brand</a>
					</div>
				</div>
				...
				<div class="card">
					<div class="card-body">
						...
						<!-- /Filter -->
						<form id="exportExcelForm" method="post" action="processor/exportbrands.php">
									...
									<tbody>
										...
										if ($result->num_rows > 0) {
											...
											while ($row = $result->fetch_assoc()) {
												...
												echo '<td>' . $ro . '</td>';
												echo '<td><a class="product-img"><img src=' . $row['img_url'] . '></a></td>';
												echo '<td>' . $row['name'] . '</td>';
												echo '<td style="white-space: pre-line;">' . $row['description'] . '</td>';
												echo "<td>
													<a class='me-3' href='editbrand.php?id=" . $row['id'] . "&uid=" . $NewId . "'>
	...
	<script>
		$(function() {
			$('#print').click(function() {
				...
				_head.find('title').text("Brand List - Print View");
	...
	<script>
		$(function() {
			$('#pdf').click(function() {
				...
				// Use html2pdf library to convert the content into a PDF
				var pdfOptions = {
					margin: 10,
					filename: 'brandlist.pdf',
					image: {
						type: 'jpeg',
						quality: 0.98
					},
	...
	<script>
		$(document).ready(function() {
			$('#export-excel').click(function(e) {
				...
				var form = $('<form action="processor/exportbrands.php" method="post" style="display: none;">');
		...
		$(".ajax-delete-btn").on("click", function() {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					$.ajax({
						url: "./processor/deletebrands.php", // Replace with the correct URL
						type: "POST",
						data: {
							id: id
						},
